...
def process_results(channel, sender, board, string, results_queue):
    ...
    while results_queue.qsize() > 0:
        posts.append(results_queue.get())
    if len(posts) <= 0:
...
def search_catalog(channel, sender, board, string):
    ...
    for page in catalog_json:
        t = Thread(None, target=search_page, args=(mutex, results_queue, page, search_specifics))
        t.start()
